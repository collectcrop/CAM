...
std::vector<RangeQ> load_ranges(const std::string& filename) {
    std::vector<RangeQ> queries;
    ...
    RangeQ rq;
    while (fin.read(reinterpret_cast<char*>(&rq), sizeof(RangeQ))) {
...
bool binary_search_record(Record* records, size_t lo, size_t hi, KeyType target_key){
    size_t left = lo;
    size_t right = hi;
    while (left <= right) {
...
std::vector<RangeQ> load_ranges_pgm_safe(const std::string& filename) {
    std::ifstream in(filename, std::ios::binary);
    if (!in) {
        throw std::runtime_error("Failed to open range file: " + filename);
    }

    std::vector<RangeQ> ranges;
    ...
    while (in.read(reinterpret_cast<char*>(&q), sizeof(RangeQ))) {
        ++total;
