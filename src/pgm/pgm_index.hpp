...
public:
    ...
    /**
     * Estimate the page window [page_lo, page_hi] that may contain the key.
     * Purely compute from segments; does not touch cache or disk.
     */
    std::pair<size_t, size_t> estimate_pages_for_key(const K& key) const {
        auto k = std::max(first_key, key);
        auto it = segment_for_key(k); // uses internal levels; O(log #segments)
    ...
    /**
     * Estimate page window for a range [lo_key, hi_key].
     */
