...
uint64_t lookup_with_leaf(uint64_t key, size_t* err, size_t* leaf) {{
  ...
  *err = {};
  uint64_t start = _rmi_lookup_pre_cachefix_with_leaf(key, &error_on_spline_search, leaf);
...
inline size_t FCLAMP(double inp, double bound) {{
  ...
  return (inp > bound ? bound : (size_t)inp);
    ...
    let rmi_lookup_name = if rmi.cache_fix.is_none() {
        ...
        "_rmi_lookup_pre_cachefix_with_leaf"
    };
    let rmi_lookup_wrapper_name = if rmi.cache_fix.is_none() {
        "lookup"
    } else {
        "_rmi_lookup_pre_cachefix"
    };
    
    let lookup_sig_with_leaf = if report_last_layer_errors {
        format!("uint64_t {}({} key, size_t* err, size_t* leaf)", rmi_lookup_name, key_type.c_type())
    } else {
        format!("uint64_t {}({} key, size_t* leaf)", rmi_lookup_name, key_type.c_type())
    };
    let lookup_sig = if report_last_layer_errors {
        format!("uint64_t {}({} key, size_t* err)", rmi_lookup_wrapper_name, key_type.c_type())
    } else {
        format!("uint64_t {}({} key)", rmi_lookup_wrapper_name, key_type.c_type())
    };
    writeln!(code_output, "{} {{", lookup_sig_with_leaf)?;

    let mut needed_vars = HashSet::new();
    let has_multi_model_layer = rmi.rmi.iter().any(|layer| layer.len() > 1);
    if has_multi_model_layer {
    ...
    if has_multi_model_layer {
        writeln!(code_output, "  if (leaf) *leaf = modelIndex;")?;
    } else {
        writeln!(code_output, "  if (leaf) *leaf = 0;")?;
    }
    ...
    if report_last_layer_errors {
        writeln!(code_output, "  return {}(key, err, nullptr);", rmi_lookup_name)?;
    } else {
        writeln!(code_output, "  return {}(key, nullptr);", rmi_lookup_name)?;
    }
    writeln!(code_output, "}}")?;
    ...
    if rmi.cache_fix.is_none() {
        writeln!(header_output, "{};", lookup_sig_with_leaf)?;
        writeln!(header_output, "{};", lookup_sig)?;
    } else {
        writeln!(header_output, "uint64_t lookup_with_leaf(uint64_t key, size_t* err, size_t* leaf);")?;
