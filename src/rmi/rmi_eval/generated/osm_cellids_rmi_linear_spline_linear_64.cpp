#include "osm_cellids_rmi_linear_spline_linear_64.h"
#include "osm_cellids_rmi_linear_spline_linear_64_data.h"
#include <math.h>
#include <cmath>
#include <fstream>
#include <filesystem>
#include <iostream>
namespace osm_cellids_rmi_linear_spline_linear_64 {
bool load(char const* dataPath) {
  {
    std::ifstream infile(std::filesystem::path(dataPath) / "osm_cellids_rmi_linear_spline_linear_64_L1_PARAMETERS", std::ios::in | std::ios::binary);
    if (!infile.good()) return false;
    L1_PARAMETERS = (char*) malloc(1536);
    if (L1_PARAMETERS == NULL) return false;
    infile.read((char*)L1_PARAMETERS, 1536);
    if (!infile.good()) return false;
  }
  return true;
}
...
uint64_t lookup_with_leaf(uint64_t key, size_t* err, size_t* leaf) {
  ...
  modelIndex = FCLAMP(fpred, 64.0 - 1.0);
