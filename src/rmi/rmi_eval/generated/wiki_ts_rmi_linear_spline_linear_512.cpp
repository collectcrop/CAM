#include "wiki_ts_rmi_linear_spline_linear_512.h"
#include "wiki_ts_rmi_linear_spline_linear_512_data.h"
#include <math.h>
#include <cmath>
#include <fstream>
#include <filesystem>
#include <iostream>
namespace wiki_ts_rmi_linear_spline_linear_512 {
bool load(char const* dataPath) {
  {
    std::ifstream infile(std::filesystem::path(dataPath) / "wiki_ts_rmi_linear_spline_linear_512_L1_PARAMETERS", std::ios::in | std::ios::binary);
...
uint64_t lookup_with_leaf(uint64_t key, size_t* err, size_t* leaf) {
  ...
  modelIndex = FCLAMP(fpred, 512.0 - 1.0);
