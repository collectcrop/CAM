...
SEED="${SEED:-42}"
ESTIMATE_RANDOMIZE_DATASETS="${ESTIMATE_RANDOMIZE_DATASETS:-1}"
ESTIMATE_WARMUP_REPEATS="${ESTIMATE_WARMUP_REPEATS:-1}"
ESTIMATE_TIMING_REPEATS="${ESTIMATE_TIMING_REPEATS:-3}"
ESTIMATE_SHUFFLE_SEED="${ESTIMATE_SHUFFLE_SEED:-$SEED}"
...
echo "[config] ESTIMATE_RANDOMIZE_DATASETS=$ESTIMATE_RANDOMIZE_DATASETS ESTIMATE_WARMUP_REPEATS=$ESTIMATE_WARMUP_REPEATS ESTIMATE_TIMING_REPEATS=$ESTIMATE_TIMING_REPEATS ESTIMATE_SHUFFLE_SEED=$ESTIMATE_SHUFFLE_SEED"
...
if [ "$SKIP_ESTIMATE" != "1" ]; then
  ...
  estimate_args+=(
    ...
    --shuffle-seed "$ESTIMATE_SHUFFLE_SEED"
  )
  if [ "$ESTIMATE_RANDOMIZE_DATASETS" = "1" ]; then
    estimate_args+=(--shuffle-datasets)
  fi
