...
[[noreturn]] void usage_error(const std::string& msg) {
    throw std::invalid_argument(
        msg +
        "\nUsage: ./pgm_range_cache_simulate --data <file> --queries <range-file>"
        " [--keys <n>] [--M <MiB>]"
        " [--epsilons <e1,e2,...> | --epsilon-start <s> --epsilon-end <e> --epsilon-step <d>]"
        " [--policies <fifo,lru,lfu,none|all>] [--budget-mode <raw|estimated|measured>]"
        " [--summary-out <csv>] [--query-limit <n>]"
    );
}
...
inline void simulate_range_query_all_at_once(
    const IndexT& index,
    ICache& cache,
    RangeQ range,
    SummaryRow& row,
    size_t total_pages)
{
    ...
    if (range.hi < range.lo) {
        std::swap(range.lo, range.hi);
    }

    auto [lo_page_lo, lo_page_hi] = index.estimate_pages_for_key(range.lo);
    auto [hi_page_lo, hi_page_hi] = index.estimate_pages_for_key(range.hi);
    size_t page_lo = std::min(lo_page_lo, hi_page_lo);
    size_t page_hi = std::max(lo_page_hi, hi_page_hi);
    ...
    size_t total_pages)
{
    ...
    row.queries = ranges.size();

    auto cache = MakeCache(policy, row.cache_bytes, PAGE_SIZE);

    const auto t0 = Clock::now();
    for (const RangeQ& range : ranges) {
        row.query_checksum += range.lo + range.hi;
        simulate_range_query_all_at_once(index, *cache, range, row, total_pages);
    }
    ...
    try {
        ...
        for (size_t epsilon : cfg.epsilons) {
            dispatch_epsilon(epsilon, [&](auto eps_tag) {
                ...
                run_epsilon<Eps>(cfg, data, ranges, *summary_out);
            });
        }
        return 0;
